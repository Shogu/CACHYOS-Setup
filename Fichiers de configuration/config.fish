...
# remplacer gedit par sudo gnome-text-editor, sans spam
function sudo --description "sudo wrapper pour gedit"
    ...
    switch $argv[1]
        case gedit
            sudo -E env DBUS_SESSION_BUS_ADDRESS= \
                DISPLAY="$DISPLAY" XAUTHORITY="$XAUTHORITY" \
                gnome-text-editor $argv[2..-1] 2>/dev/null
        case '*'
            /usr/bin/sudo $argv
    end
end
